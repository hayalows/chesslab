...
export function createPostGameSummary(args: {
  ...
  const forcedMateSwing = Boolean(topReply?.mate !== undefined || (topReply && Math.abs(topReply.score) > 90_000));
  const headline = result === "win" ? "You converted the game. Keep the good habits."
  ...
  const keyMoment = worst && topReply && forcedMateSwing
    ? `${worst.move} was the turning point. Stockfish found ${topReply.san}, which starts a forced checkmate sequence. Once mate is forced, a pawn score is no longer a useful way to describe the position.`
    : worst && topReply
