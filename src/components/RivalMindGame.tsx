      ...
      <section className={styles.workspace} id="top">
        <aside className={styles.leftRail}>
          <div className={styles.panel}>
            ...
            <details className={styles.strengthHelp}><summary>How Rival strength works</summary><p>Rated levels use Stockfish&apos;s limited-strength setting. Elo is approximate in short games. Master removes the limit, and search time shortens only when the Rival&apos;s own clock is low.</p></details>
          </div>
      ...
      {setupOpen && (
        <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => { if (hasResumableGame && event.target === event.currentTarget) closeGameSetup(); }}>
          <section className={styles.setupCard} role="dialog" aria-modal="true" aria-labelledby="setup-title" aria-describedby="setup-description">
            ...
            <label className={styles.setupField} htmlFor="setup-difficulty"><span>Rival strength</span><select id="setup-difficulty" value={difficulty} onChange={(event) => setDifficulty(event.target.value as Difficulty)}>{DIFFICULTIES.map((level) => <option key={level} value={level}>{level === "adaptive" ? `Adaptive · current level ${profile.adaptiveLevel}` : DIFFICULTY_PRESETS[level].elo === null ? `${level[0].toUpperCase() + level.slice(1)} · Maximum strength` : `${level[0].toUpperCase() + level.slice(1)} · ${DIFFICULTY_PRESETS[level].elo} Elo`}</option>)}</select><small>{difficulty === "adaptive" ? "Uses your unassisted move quality and recent results to choose the next challenge." : `${DIFFICULTY_PRESETS[difficulty].description} Rated modes use Stockfish's limited-strength setting; Elo is approximate in short games. Master removes the limit.`}</small></label>
            </div>
