...
type Profile = { display_name: string; rating: number; total_games: number; wins: number; losses: number; draws: number; adaptive_level: number; training_points: number; training_minutes: number; current_streak: number; best_streak: number };
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const [profileResult, gamesResult, ratingsResult, statsResult, achievementsResult] = await Promise.all([
        supabase.from("profiles").select("display_name,rating,total_games,wins,losses,draws,adaptive_level,training_points,training_minutes,current_streak,best_streak").single(),
  ...
  return <main className={styles.shell}>
    <header><Link href="/">← Home</Link><span>RivalMind training</span><button type="button" onClick={() => void createClient()?.auth.signOut().then(() => location.assign("/"))}>Sign out</button></header>
    ...
    <section className={styles.metrics}><article><span>Training points</span><b>{profile?.training_points ?? 0}</b><small>{profile?.training_minutes ?? 0} focused minutes</small></article><article><span>Games completed</span><b>{profile?.total_games ?? games.length}</b><small>{profile?.wins ?? 0} wins · {profile?.draws ?? 0} draws</small></article><article><span>Adaptive rival</span><b>{profile?.adaptive_level ?? 4}<i>/10</i></b><small>About {adaptiveElo} Stockfish Elo</small></article><article><span>Playing style</span><b className={styles.styleValue}>{stats?.analyzed_games && stats.analyzed_games >= 20 ? stats.style_label || "Balanced builder" : "Still learning"}</b><small>{Math.max(0, 20 - (stats?.analyzed_games ?? games.length))} games until style insight</small></article></section>
    <section className={styles.adaptivePath}><div><span>YOUR CURRENT CHALLENGE</span><h2>Level {profile?.adaptive_level ?? 4} · about {adaptiveElo} Elo</h2><p>RivalMind reviews up to five recent results and waits at least three games between changes. One difficult game never drops your level.</p></div><div className={styles.levelDots}>{Array.from({length:10},(_,index)=><i key={index} data-reached={index < (profile?.adaptive_level ?? 4)} />)}</div></section>
    <section className={styles.grid}>
      ...
      <article className={`${styles.card} ${styles.analyticsCard}`}><div className={styles.cardTitle}><div><span>Learning analytics</span><h2>How you train</h2></div></div><div className={styles.analyticsGrid}><span>Move quality<b>{stats?.average_accuracy == null ? "—" : `${Math.round(stats.average_accuracy)}%`}</b><small>From Stockfish evaluation changes</small></span><span>Thinking per game<b>{averageThinkMs ? `${Math.round(averageThinkMs / 60000)} min` : "—"}</b><small>Your active decision time</small></span><span>Coach requests<b>{totalCoachUses}</b><small>{noCoachGames} independent games</small></span><span>Best streak<b>{profile?.best_streak ?? 0}</b><small>Current: {profile?.current_streak ?? 0}</small></span></div></article>
