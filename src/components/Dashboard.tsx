...
export default function Dashboard() {
  ...
  return <main className={styles.shell}>
    ...
    <section className={styles.metrics}><article><span>Estimated strength</span><b>{profile?.estimated_strength ?? 900}</b><small>From {profile?.independent_moves ?? 0} unassisted moves</small></article><article><span>Independent quality</span><b>{profile?.independent_moves ? `${Math.round(profile.independent_accuracy)}%` : "—"}</b><small>Coach-aided moves excluded</small></article><article><span>Adaptive rival</span><b>{profile?.adaptive_level ?? 4}<i>/10</i></b><small>About {adaptiveElo} Stockfish Elo</small></article><article><span>Playing style</span><b className={styles.styleValue}>{stats?.analyzed_games && stats.analyzed_games >= 20 ? stats.style_label || "Balanced builder" : "Still learning"}</b><small>{Math.max(0, 20 - (stats?.analyzed_games ?? games.length))} games until style insight</small></article></section>
    ...
    <section className={styles.grid}>
      ...
      <article className={`${styles.card} ${styles.analyticsCard}`}><div className={styles.cardTitle}><div><span>Learning analytics</span><h2>How you train</h2></div></div><div className={styles.analyticsGrid}><span>Independent quality<b>{stats?.average_accuracy == null ? "—" : `${Math.round(stats.average_accuracy)}%`}</b><small>Only moves made without the coach</small></span><span>Thinking per game<b>{averageThinkMs ? `${Math.round(averageThinkMs / 60000)} min` : "—"}</b><small>Your active decision time</small></span><span>Coach requests<b>{totalCoachUses}</b><small>{noCoachGames} independent games</small></span><span>Best streak<b>{profile?.best_streak ?? 0}</b><small>Current: {profile?.current_streak ?? 0}</small></span></div></article>
